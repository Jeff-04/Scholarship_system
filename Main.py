...
if 'data' not in st.session_state:
    st.session_state['data'] = pd.DataFrame(columns=['KIP', 'Penghasilan Orang Tua', 'Status Orang Tua Siswa', 'Data Dinas Sosial', 'Prediksi'])
...
modul_beasiswa.add_bg_from_local('Images/background.jpg')   
...
if selected == "Beranda":
        ...
        if data_list_beasiswa != False:
            for i in range(len(data_list_beasiswa)):
                ...
                with st.expander("Selengkapnya .."):
                    ...
                    close_date = str(data_list_beasiswa[i][2]).split('-')
                    close_date = date(int(close_date[0]), int(close_date[1]), int(close_date[2]))

                    open_date = str(data_list_beasiswa[i][1]).split('-')
                    open_date = date(int(open_date[0]), int(open_date[1]), int(open_date[2]))


                    if  today >= open_date and today <= close_date and st.session_state['login_user'] != '' and data_pendaftar == True:
    ...
    with col2:
        ...
        st.image("Images/logo_sekolah.png", width=475)
...
#             b = st.button("test")
        ...
        if option == "Signup":
            ...
            with placeholder_2.form("signup"):
                ...
                opsi_pendapatan = st.number_input("Penghasilan Orang Tua / Wali (Rupiah)", min_value=0, max_value=100000000, step=1000)
                # opsi_pendapatan = st.selectbox(
                #     'Penghasilan Orang Tua / wali',
                #     ('Kurang dari Rp 1.000.000','Diatas Rp 1.000.000', 'Tidak berpenghasilan'),
                # )
                ...
                if submit_signup:
                    # Rubah Angka -> Kategori
                    if int(opsi_pendapatan) == 0:
                        opsi_pendapatan = "Tidak berpenghasilan"

                    elif int(opsi_pendapatan) >= 1000000:
                        opsi_pendapatan = "Kurang dari Rp 1.000.000"
                    
                    else:
                        opsi_pendapatan = "Kurang dari Rp 1.000.000"

                    if alamat == '' or nama == '' or password == '':
    ...
    with col2:
        ...
        if option == "Biodata" :
            ...
            with placeholder_3.form("biodata_diri"):
                ...
                if data_list[6] == "Kurang dari Rp 1.000.000":
                    index_pendapatan = 0
                if data_list[6] == 'Diatas Rp 1.000.000':
                    index_pendapatan = 1
                if data_list[6] == 'Tidak berpenghasilan':
                    index_pendapatan = 2
...
if selected == 'Data Siswa':
    ...
    with col3 :
        ...
        if option_data == 'Ubah Data':
            ...
            if selected_penghasilan == "Kurang dari Rp 1.000.000":
                index_pendapatan = 0
            ...
            with placeholder_7.form("insert"):
                ...
                if submit_signup:
                    if int(opsi_pendapatan) == 0:
                        ...
                        opsi_pendapatan = "Diatas Rp 1.000.000"
...
if selected == "Testing Model":
    def change(kip_inp, penghasilan_inp, statusortu_inp, datadinas_inp):
        # KIP
        if kip_inp == "Ada":
            kip_inp = 1
        else:
        ...
        # penghasilan_inp
        if penghasilan_inp == "Kurang dari 1 Juta":
            penghasilan_inp = 0.75
        elif penghasilan_inp == "diatas 1 Juta":
            penghasilan_inp = 0.5
        else:
            penghasilan_inp = 1

        # statusortu_inp
        if statusortu_inp == "Yatim / Piatu":
            statusortu_inp = 0.75
        elif statusortu_inp == "Yatim Piatu":
            statusortu_inp = 1
        else:
            statusortu_inp = 0.5

        # datadinas_inp
        if datadinas_inp == "Rentang Miskin":
            datadinas_inp = 0.75
        elif datadinas_inp == "Miskin":
            datadinas_inp = 1
        else:
            datadinas_inp = 0.5
        
        return kip_inp, penghasilan_inp, statusortu_inp, datadinas_inp

    st.markdown("<h2 align='center'> Testing SVM dan SMART </h2>", unsafe_allow_html=True)
    buffer, col2, col3 = st.columns([2,6,2]) 
    with col2:   
        form_test = st.empty()

        with form_test.form('testing'):
            kip = st.selectbox("Kartu Indonesia Pelajar", ['Ada', 'Tidak Ada'])
            penghasilan_ortu = st.selectbox("Penghasilan Orang Tua", [
                'Kurang dari 1 Juta',
                'diatas 1 Juta',
                'Tidak Berpenghasilan'
            ])
            status_ortu = st.selectbox("Status Orang Tua Siswa", [
                'Yatim / Piatu',
                'Yatim Piatu',
                'Lengkap'
            ])
            data_dinas = st.selectbox("Data Dinas Sosial", [
                'Rentang Miskin',
                'Miskin',
                'Normal'
            ])
            ...
            if submit:
                data = {
                    ...
                    'Data Dinas Sosial' : data_dinas
                }

                kip, penghasilan_ortu, status_ortu, data_dinas = change(kip, penghasilan_ortu, status_ortu, data_dinas)
                loaded_model = pickle.load(open('Models/model_beasiswa_svm.pkl', 'rb'))
                predict = loaded_model.predict([kip, penghasilan_ortu, status_ortu, data_dinas])

                if predict != 1:
                    predict = 0

                data['Prediksi'] = int(predict)
                st.session_state['data'] = st.session_state['data'].append(data, ignore_index=True)


        st.write("## Data hasil SVM")
        st.session_state['data'] = st.session_state['data'].astype({'Prediksi' : int})
        st.table(st.session_state['data'])

        st.write("## Data hasil SMART")
        if len(st.session_state['data']) > 0:
            new_df = st.session_state['data'].copy()
            new_df = new_df.loc[new_df['Prediksi'] > 0]

            # Konversi ke numerik
            new_df_1 = new_df.copy()

            new_df_1["KIP"].replace({"Ada": "1", "Tidak Ada": "0"}, inplace=True)
            new_df_1 = new_df_1.astype({'KIP' : int})

            new_df_1["Penghasilan Orang Tua"].replace({
                "Kurang dari 1 Juta" : "0.75",
                "diatas 1 Juta" : "0.5",
                "Tidak Berpenghasilan" : "1"
            },
                inplace=True)
            
            new_df_1 = new_df_1.astype({'Penghasilan Orang Tua' : float})

            new_df_1["Status Orang Tua Siswa"].replace({"Yatim / Piatu": "0.75", "Yatim Piatu" : "1", "Lengkap" : "0.5"}, inplace=True)
            new_df_1 = new_df_1.astype({'Status Orang Tua Siswa' : float})

            new_df_1["Data Dinas Sosial"].replace({"Miskin": "1", "Rentang Miskin": "0.75", "Normal" : "0.5"}, inplace=True)
            new_df_1 = new_df_1.astype({'Data Dinas Sosial' : float})

            # Menghitung Utility
            new_df_1['KIP'] = new_df_1['KIP'].apply(lambda x: int((x - 0) / 1))
            new_df_1['Penghasilan Orang Tua'] = new_df_1['Penghasilan Orang Tua'].apply(lambda x: float((x - 0.25) / 0.75))
            new_df_1['Status Orang Tua Siswa'] = new_df_1['Status Orang Tua Siswa'].apply(lambda x: float((x - 0.5) / 0.5))
            new_df_1['Data Dinas Sosial'] = new_df_1['Data Dinas Sosial'].apply(lambda x: float((x - 0.5) / 0.5))

            # Perangkingan
            new_df_2 = new_df_1.astype(float)
            new_df_2['KIP'] = new_df_2['KIP'].apply(lambda x: float(x * 0.4))
            new_df_2['Penghasilan Orang Tua'] = new_df_2['Penghasilan Orang Tua'].apply(lambda x: float(x * 0.1))
            new_df_2['Status Orang Tua Siswa'] = new_df_2['Status Orang Tua Siswa'].apply(lambda x: float(x * 0.3))
            new_df_2['Data Dinas Sosial'] = new_df_2['Data Dinas Sosial'].apply(lambda x: float(x * 0.2))
            
            column_list = list(new_df_2)
            
            new_df_2["Total"] = new_df_2[column_list].sum(axis=1)
            new_df_2['Ranking'] = new_df_2['Total'].rank(ascending = 0, method='dense')

            new_df['Total'] = new_df_2['Total']
            new_df['Ranking'] = new_df_2['Ranking']

            new_df = new_df.astype({"Ranking" : int})
