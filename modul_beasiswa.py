...
def menu():
    if st.session_state["login_user"] == "" and st.session_state["login_admin"] == "":
        menu = [
            ["Beranda", "SMART & SVM", "Pengumuman", "Akun"],
            ["house", "info-circle", "card-checklist", "box-arrow-right"],
        ]
        return menu
    if st.session_state["login_user"] != "":
        menu = [
            ...
            ["house", "info-circle", "person", "card-checklist", "box-arrow-left"],
        ]
        return menu
...
def get_pendaftaran(id_siswa, subject_beasiswa):
    try:
        ...
        statement = (
            f"SELECT Nisn_siswa from data_siswa WHERE Nama_siswa = '{id_siswa}';"
        )
        cursor.execute(statement)
...
def cek_pendaftar(beasiswa_inp):
    try:
        ...
        statement = (
            f"SELECT * from pendafaran WHERE Subject_beasiswa = '{beasiswa_inp}';"
        )
        cursor.execute(statement)
        rows = cursor.fetchall()
        if not rows:
...
def sistem_svm(df, df2):
    ...
    st.markdown(
        "<h1 style='text-align:center'; color: black;'>Klasifikasi SVM</h1>",
        unsafe_allow_html=True,
    )
    st.table(df2)
    df_new = df.loc[df["predict_svm"] == "Lolos Seleksi 1"]
...
def sistem_smart(df):
    ...
    df_new_2["Kartu Indonesia Pelajar"].replace(
        {"Ada": "1", "Tidak ada": "0"}, inplace=True
    )
    df_new_2 = df_new_2.astype({"Kartu Indonesia Pelajar": int})

    df_new_2["Penghasilan Orang Tua"].replace(
        {
            "Kurang dari Rp 1.000.000": "0.75",
            "Diatas Rp 1.000.000": "0.5",
    ...
    df_new_2["Status Orang Tua"].replace(
        {"Yatim / Piatu": "0.75", "Yatim Piatu": "1", "Lengkap": "0.5"},
        inplace=True,
    )
    df_new_2 = df_new_2.astype({"Status Orang Tua": float})

    df_new_2["Data Dinas Sosial"].replace(
        {
            "Miskin": "1",
            "Rentan Miskin": "0.75",
            "Normal": "0.5",
    ...
    st.markdown(
        "<h1 style='text-align:center'; color: black;'>SISTEM SMART</h1>",
        unsafe_allow_html=True,
    )
    st.table(df2)
